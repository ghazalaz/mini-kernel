...
uint32_t kmalloc_int(uint32_t sz, int align, uint32_t *phys)
{
    if (kheap != 0)
    {
        void *addr = my_alloc(sz, (uint8_t)align, kheap);
        if (phys != 0)
        {
...
heap_t* init_heap(int size){
    ...
    uint32_t start = kmalloc(size);
    uint32_t end_addr = start + size;
...
void *my_alloc(uint32_t size, uint8_t page_align, heap_t *heap)
{
    ...
    if (iterator == BAD) // If we didn't find a suitable hole
    {
        ...
        return my_alloc(size, page_align, heap);
    }
...
void my_free(void *p, heap_t *heap)
{
